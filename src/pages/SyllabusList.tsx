...
import { 
  FileText, Search, Download, Loader2, Briefcase,
  ChevronRight, BookOpen, Sparkles, Database, 
  ShieldCheck, GraduationCap, ArrowUpRight, History,
  Link
} from "lucide-react";
  ...
  useEffect(() => {
    const fetchSyllabus = async () => {
      try {
        ...
        console.error("Syllabus Registry Error:", err);
        setSyllabus([]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="bg-[#f8fafc] min-h-screen pb-20">
      ...
      <section className="relative bg-slate-900 pt-20 pb-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center space-y-6">
          ...
          >
            <GraduationCap size={14} /> Curriculum Database
          </motion.div>
          ...
          >
            EXAM <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400 font-black italic">SYLLABUS</span>
          </motion.h1>
          ...
          >
            Access the most recent and detailed exam patterns for Central and State Government examinations.
          </motion.p>
        </div>
      </section>
      ...
      <div className="max-w-3xl mx-auto px-4 -mt-10 relative z-20">
        ...
        >
          ...
          <div className="absolute right-6 top-1/2 -translate-y-1/2 hidden md:flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-2xl border border-slate-100">
             ...
             <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{filteredSyllabus.length} Indices Found</span>
          </div>
        </motion.div>
      </div>

      {/* 3. Main Syllabus Grid */}
        ...
        <AnimatePresence mode="wait">
          {loading ? (
                ...
                >
                  ...
                  <div className="relative z-10 space-y-6">
                    <div className="flex items-start justify-between">
                      <div className="h-14 w-14 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-500 shadow-inner border border-indigo-100">
                        <BookOpen size={24} />
                      </div>
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">
                           REV: {formatDate(item.created_at)}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {item.category?.name && (
                        <div className="inline-flex px-3 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-black uppercase tracking-tighter">
                          {item.category.name}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="relative z-10 pt-8 mt-auto border-t border-slate-50 flex items-center gap-3">
                    {item.pdf_link ? (
                      <a
                        href={item.pdf_link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex-1 inline-flex items-center justify-center gap-3 px-8 py-4 bg-slate-900 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200 active:scale-95 group/btn"
                      >
                        DOWNLOAD PDF
                        <Download size={18} className="group-hover/btn:translate-y-1 transition-transform" />
                      </a>
                    ) : (
                      <div className="flex-1 py-4 text-center text-[10px] font-black text-slate-300 uppercase bg-slate-50 rounded-2xl italic">
                        PDF Coming Soon
                      </div>
                    )}
                    <Link
                      to={`/syllabus/${item._id || item.id}`}
                      className="h-12 w-12 bg-slate-50 hover:bg-white border border-transparent hover:border-slate-200 text-slate-400 hover:text-indigo-600 rounded-2xl flex items-center justify-center transition-all shadow-sm active:scale-90"
                    >
            ...
            >
              ...
              <h3 className="text-3xl font-black text-slate-900 mb-2 uppercase tracking-tighter">Index Empty</h3>
              <p className="text-slate-400 font-medium max-w-sm mx-auto">No syllabus documents matched your search term. Please check back later.</p>
              <button 
        ...
        <div className="bg-slate-900 rounded-[50px] p-10 lg:p-16 text-white relative overflow-hidden group shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12 text-center lg:text-left">
            <div className="space-y-4 max-w-xl">
              ...
              <h4 className="text-4xl font-black italic tracking-tighter leading-none">Can't find a syllabus?</h4>
              <p className="text-slate-400 font-medium text-lg leading-relaxed">
                Contact our support team with the exam details, and we will update the index with the latest official pattern within 24 hours.
              </p>
            </div>
